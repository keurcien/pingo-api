...
class RecetteMarmiton(RecetteBase):
    ...
    def _get_servings(self, soup):
        self._servings = soup.find(
            "span", {"class": "recipe-infos__quantity__value"}
        ).text
    ...
    def _get_ingredients(self, soup):
        ...
        for ingredient in ingredients:
            ...
            quantity = ingredient.find(
                "span", {"class": "recipe-ingredient-qt"}
            ).text.strip()
            complement = ingredient.find(
                "span", {"class": "recipe-ingredient__complement"}
            ).text.strip()
